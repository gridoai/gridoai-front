  ...
  features: [
    {
      ...
      image: <div>
        <Message content={`What were the action items from our last team meeting?`} type={`userMessage`} sources={``} loading={false} index={0} />
        <Message content={`The action items from our last team meeting were: 1) Jane to finalize the budget proposal, 2) Mike to schedule a follow-up meeting with the client, and 3) Sarah to update the project timeline.`} type={`robot`} sources={`Team_Meeting_Minutes.pptx`} loading={false} index={1} />
        <div className="hidden md:block">
          <Message content={`What are the key updates expected in the project timeline that Sarah needs to make?`} type={`userMessage`} sources={``} loading={false} index={1} />
          <Message content={`Sarah needs to update the project timeline to reflect the new product launch date and the revised marketing campaign schedule`} type={`robot`} sources={`Project_Timeline.docx`} loading={false} index={1} />
        </div>
      </div>
    },
    {
    ...
    },
    {
      ...
      image: <Image src={orgMembers} alt="" className="rounded-xl overflow-hidden" />
        ...
        <main className="xl:max-w-7xl mx-auto   to-background">
          <section className="px-8 py-20">
            ...
            <div className="flex flex-col gap-16 md:gap-8">
              {
                landingPageContent.features.map((feature, index) =>
                  <AnimatedComponent
                    threshold={0.7}
                    key={index} animationClass="animate-in fade-in opacity-100 ease-in" className="opacity-0 duration-10000"
                  >
                    <div className={`flex ${index % 2 !== 0 ? `md:flex-row-reverse` : `md:flex-row`} flex-col items-center md:min-h-[750px] flex-1 gap-8`}>
            ...
            {/* <div className="flex gap-2 overflow-y-scroll">
              ...
              (
                <div key={useCase.title} className="flex min-w-[400px] aspect-square flex-col gap-4 cursor-pointer  transition-all p-4 md:p-10 mb-6 bg-gradient-to-br from-surface-gradient-from to-surface-gradient-to rounded-xl">
                  <div className="flex flex-col gap-1 items-center">
                    <h1 className="md:text-4xl text-2xl font-bold text-center ">{useCase.title}</h1>
                    <div className="text-xl opacity-75">{useCase.description}</div>
                  </div></div>
              )
              )}
            </div> */}
      ...
      {items.map((feature, index) => (
        ...
        >
          <div className="break-inside-avoid flex flex-col gap-4 cursor-pointer  transition-all p-4 md:p-10 mb-6 bg-gradient-to-br from-surface-gradient-from to-surface-gradient-to rounded-xl">
            ...
            <p className="text-xl opacity-75 text-center">{feature.description}</p>
          </div>
        </AnimatedComponent>
